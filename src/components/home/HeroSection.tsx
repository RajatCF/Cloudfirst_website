...
const HeroSection = () => {
  ...
  return (
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden" style={{ background: '#0c0e1a' }}>
      ...
      {/* Overlay — left side readable, right side clear for 3D model */}
      <div className="absolute inset-0 z-[1]" style={{
        background: 'linear-gradient(110deg, rgba(12,14,26,0.82) 0%, rgba(12,14,26,0.42) 50%, rgba(12,14,26,0.00) 100%)',
        pointerEvents: 'none',
      }} />

      {/* ── LOADER ── */}
      {loading && (
        <div className="absolute inset-0 flex items-center justify-center z-50" style={{ background: '#0c0e1a' }}>
          <div className="relative flex items-center justify-center" style={{ width: 240, height: 240 }}>
            ...
            <div className="relative z-10 flex gap-0.5" style={{
              fontFamily: 'Space Grotesk, sans-serif',
              fontWeight: 800, fontSize: '1.5rem',
              letterSpacing: '0.1em', color: '#a5b4fc',
            }}>
              {'CLOUDFIRST'.split('').map((c, i) => (
                <span key={i} style={{ opacity: 0.2, animation: `loader-cf 2s ${i * 0.08}s infinite` }}>{c}</span>
              ))}
            </div>
          </div>
          <style>{`
            ...
            @keyframes loader-ring {
              0%   { box-shadow: 0 0 0 0 transparent, 0 0 60px 18px rgba(99,102,241,0.3),  0 0 120px 40px rgba(59,130,246,0.18); }
              50%  { box-shadow: 0 0 0 0 transparent, 0 0 80px 28px rgba(124,58,237,0.28), 0 0 160px 60px rgba(99,102,241,0.14); }
              100% { box-shadow: 0 0 0 0 transparent, 0 0 60px 18px rgba(99,102,241,0.3),  0 0 120px 40px rgba(59,130,246,0.18); }
            }
        ...
        <div className="relative z-10 w-full max-w-7xl mx-auto px-5 sm:px-8 lg:px-12 py-20 lg:py-28">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
            ...
            <div className="flex flex-col gap-7 text-center lg:text-left">
              ...
              {/* Eyebrow */}
              <div ref={taglineRef} className="flex items-center gap-2.5 justify-center lg:justify-start opacity-0">
                <span className="w-2 h-2 rounded-full" style={{ background: '#93c5fd', boxShadow: '0 0 10px rgba(147,197,253,0.9)' }} />
                <span style={{
              ...
              {/* Sub */}
              <p ref={subRef} className="opacity-0" style={{
                fontSize: '1.05rem', lineHeight: 1.75,
                color: '#cbd5e1', maxWidth: 480,
              }}>
                Enterprise-grade cloud security, compliance automation, and AI-driven data governance — trusted by 500+ organizations worldwide.
              </p>

              {/* CTAs */}
              <div ref={ctaRef} className="flex flex-col sm:flex-row gap-3 justify-center lg:justify-start opacity-0">
                <a
                  href="/contact"
              ...
              <div ref={statsRef} className="grid grid-cols-2 sm:grid-cols-4 gap-3 opacity-0">
                {STATS.map(s => (
                  <div key={s.label} style={{
                    ...
                    <div style={{ fontSize: '0.72rem', fontWeight: 500, color: '#e2e8f0', marginTop: 2 }}>{s.label}</div>
                  </div>
                ))}
              </div>
            </div>
              ...
              <div style={{
                ...
                <div style={{ marginBottom: 28 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
                    ...
                    <span style={{ fontSize: '0.68rem', fontWeight: 700, letterSpacing: '0.16em', textTransform: 'uppercase', color: '#93c5fd' }}>
                      Quick Connect
                    </span>
                  </div>
                  <h3 style={{ fontFamily: 'Space Grotesk, sans-serif', fontWeight: 900, fontSize: '1.5rem', color: '#ffffff', lineHeight: 1.2 }}>
                    Talk to an Expert
                  </h3>
                  <p style={{ fontSize: '0.84rem', color: '#cbd5e1', marginTop: 6 }}>We respond within 24 hours</p>
                </div>
                ...
                {submitted ? (
                  <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, padding: '36px 0', textAlign: 'center' }}>
                    <div style={{
                      ...
                      <svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M4 12l6 6L20 6" stroke="#6366f1" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/></svg>
                    </div>
                  ...
                  <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                    {[
                      ...
                      <div key={f.key}>
                        <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#e2e8f0', marginBottom: 6 }}>{f.label}</label>
                        <input
                          type={f.type}
                    ...
                    <div>
                      <label style={{ display: 'block', fontSize: '0.75rem', fontWeight: 600, color: '#e2e8f0', marginBottom: 6 }}>Message</label>
                      <textarea
                        value={form.message}
