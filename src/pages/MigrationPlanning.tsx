...
const MigrationPlanning: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="max-w-7xl mx-auto px-6 pt-28 pb-16 flex flex-col lg:flex-row items-center gap-12">
        <div className="flex-1">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-gray-900 leading-tight mb-5" style={{ fontFamily: "'Georgia', serif" }}>
            Move to cloud<br />
            <span className="text-bright-blue">without the risk</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={() => navigate("/contact")} className="px-6 py-3 bg-bright-blue hover:bg-bright-blue/90 text-white text-sm font-semibold rounded-lg transition-colors">Plan my migration →</button>
      ...
      {/* Stats */}
      <div className="bg-bright-blue py-10">
        <div className="max-w-6xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
          {[
            ...
            <div key={s.label}>
              ...
              <div className="text-xs text-light-blue">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="space-y-4">
          {waves.map((w, i) => (
            <div key={w.wave} className="flex gap-5 p-6 bg-gray-50 rounded-2xl border border-gray-100 hover:border-bright-blue/25 hover:bg-bright-blue/5 transition-all">
              <div className="w-12 h-12 rounded-xl bg-bright-blue/10 text-bright-blue flex flex-col items-center justify-center flex-shrink-0">
                <span className="text-[10px] font-bold uppercase tracking-wider">W{i + 1}</span>
              </div>
              <div>
                <div className="text-[11px] font-semibold text-bright-blue uppercase tracking-wider mb-1">{w.wave}</div>
              ...
              {readinessChecks.map((check) => (
                <div key={check} className="flex items-start gap-3 bg-white rounded-lg p-3 border border-gray-100">
                  <div className="w-5 h-5 rounded-full bg-bright-blue/10 flex items-center justify-center flex-shrink-0 mt-0.5">
                    <svg className="w-3 h-3 text-bright-blue" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" /></svg>
                  </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        <div className="bg-bright-blue rounded-2xl px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-8">
          <div className="max-w-lg">
            ...
            <p className="text-light-blue text-sm leading-relaxed">We'll assess your current environment, identify blockers, and produce a recommended wave plan — no commitment required.</p>
          </div>
          <div className="flex flex-col gap-3 flex-shrink-0">
            <button onClick={() => navigate("/contact")} className="px-7 py-3 bg-white hover:bg-white/95 text-bright-blue text-sm font-bold rounded-lg transition-colors whitespace-nowrap">Book an assessment →</button>
