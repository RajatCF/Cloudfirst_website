  ...
  return (
    <div className="min-h-screen bg-[#f7f8fa]">
      ...
      <div className="w-full bg-[#1e1b4b] relative overflow-hidden pt-16 lg:pt-20">
        <div className="absolute inset-0 opacity-30" style={{ background: "radial-gradient(ellipse at 60% 40%, #7c3aed 0%, transparent 60%), radial-gradient(ellipse at 20% 80%, #2563eb 0%, transparent 50%)" }} />
        <div className="max-w-4xl mx-auto px-6 pt-24 pb-20 text-center relative z-10">
          <span className="inline-block text-xs font-semibold uppercase tracking-widest text-violet-300 border border-violet-400/30 bg-violet-400/10 rounded-full px-3 py-1 mb-5">Services</span>
          <h1 className="text-4xl md:text-5xl font-bold text-white leading-tight mb-5" style={{ fontFamily: "'Georgia', serif" }}>
            A cloud strategy that <span className="text-violet-300">actually gets executed</span>
          </h1>
          ...
          <div className="flex flex-wrap gap-3 justify-center">
            <button onClick={() => navigate("/contact")} className="px-7 py-3 bg-violet-500 hover:bg-violet-400 text-white text-sm font-bold rounded-lg transition-colors">Start your roadmap →</button>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-5">
          {pillars.map((p) => (
            <div key={p.title} className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 text-center hover:shadow-md hover:border-violet-100 transition-all">
            ...
            {phases.map((ph, i) => (
              <div key={ph.phase} className="relative">
                <div className="flex items-center gap-2 mb-3">
                  <div className="w-7 h-7 rounded-full bg-violet-600 text-white text-xs font-bold flex items-center justify-center flex-shrink-0">{i + 1}</div>
                  <div>
                    <div className="text-[10px] font-semibold uppercase tracking-wider text-violet-600">{ph.phase}</div>
      ...
      <div className="max-w-6xl mx-auto px-6 pb-16">
        <div className="bg-[#1e1b4b] rounded-2xl px-8 py-12 flex flex-col md:flex-row items-center justify-between gap-8 relative overflow-hidden">
          <div className="absolute inset-0 opacity-30" style={{ background: "radial-gradient(ellipse at 90% 50%, #7c3aed 0%, transparent 60%)" }} />
          <div className="relative z-10 max-w-lg">
            <span className="inline-block text-xs font-semibold uppercase tracking-widest text-violet-400 mb-3">6-week engagement</span>
          ...
          <div className="relative z-10 flex flex-col gap-3 flex-shrink-0">
            <button onClick={() => navigate("/contact")} className="px-7 py-3 bg-violet-500 hover:bg-violet-400 text-white text-sm font-bold rounded-lg transition-colors whitespace-nowrap">Book a strategy call →</button>
